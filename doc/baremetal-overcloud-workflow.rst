...
Setting up the undercloud virthost machine for PXE forwarding
-------------------------------------------------------------

When using a baremetal undercloud machine, one NIC will be set up on the provisioning network.
That NIC will be used to PXE boot the overcloud nodes. When using a VM for the undercloud,
the virthost machine must be setup for PXE forwarding.

.. include:: <https://github.com/redhat-openstack/ansible-role-tripleo-baremetal-prep-virthost/blob/master/templates/add-provisioning-interface.sh.j2>
